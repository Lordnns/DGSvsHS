...
#   - dotnet SDK 10 (for `dotnet publish -r linux-musl-arm64`)
...
echo "==> Publishing Arch Server (linux-musl-arm64, self-contained)..."
...
RUN mkdir -p /opt/dotnet_extract && \
    cd /opt/app && \
    (timeout 10 ./DGSvsHS.ArchServer >/dev/null 2>&1 || true) && \
...
log "pre-extracted libmsquic-openssl.so:"
find /opt/dotnet_extract -name 'libmsquic-openssl.so' -print -exec ls -la {} \; 2>/dev/null
log "libmsquic on system:"
ls -la /usr/lib/libmsquic* 2>&1
log "===== diag end ====="

log "launching DGSvsHS.ArchServer"
log "Output -> /tmp/arch.log (tail -f /tmp/arch.log from shell)"
...
(
    wait $ARCH_PID
    AEXIT=$?
    echo "[init] ArchServer exited code $AEXIT — see /tmp/arch.log" > /dev/kmsg
) &
